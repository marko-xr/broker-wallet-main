...
import '../../../viewmodels/ListScreens/list_brokers_viewmodel.dart';
import '../../../services/phone_input_service.dart';

class BrokersListView extends StatefulWidget {
  const BrokersListView({super.key});

  @override
  State<BrokersListView> createState() => _BrokersListViewState();
}

class _BrokersListViewState extends State<BrokersListView> {
  @override
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider(
      create: (_) => BrokersListViewModel(),
      child: Consumer<BrokersListViewModel>(
        builder: (context, vm, _) {
          return Scaffold(
            ...
            floatingActionButton: FloatingActionButton(
              onPressed: () => context.push('/add-brokers'),
            ...
            body: StreamBuilder<List<BrokerModel>>(
              ...
              builder: (context, snapshot) {
                ...
                return _buildBrokersListWithDateSeparators(
  ...
  Widget _buildBrokersListWithDateSeparators(
    List<BrokerModel> brokers,
    BrokersListViewModel vm,
    AppLocalizations loc,
    ColorScheme colors,
    TextTheme texts,
  ) {
    // Group brokers by week
    final groupedBrokers = _groupBrokersByWeek(brokers);
        ...
        if (brokers.isNotEmpty)
          SliverToBoxAdapter(
            child: Padding(
              ...
              child: _buildStatisticsCard(brokers, colors, texts, loc),
            ),
          ),

        // Grouped Brokers List
        ...groupedBrokers.entries.map((entry) {
          final weekKey = entry.key;
          final weekBrokers = entry.value;
          ...
          return SliverList(
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                ...
                // Adjust index for broker items
                final brokerIndex = index - 1;
                final broker = weekBrokers[brokerIndex];

                return Padding(
                  padding:
                      const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                  child: _EnhancedBrokerTile(
                    key: ValueKey('broker_${broker.id ?? brokerIndex}'),
                    broker: broker,
                    onDelete: () =>
                        _showDeleteConfirmation(broker, vm, loc, colors),
                    onEdit: () {
                      // Navigate to edit if needed
                    },
                    onCall: () => _makePhoneCall(broker.phoneNumber),
                    onWhatsApp: () =>
                        _openWhatsApp(broker.phoneNumber, broker.name),
                    loc: loc,
                    index: brokerIndex,
                  ),
                );
              },
              childCount: weekBrokers.length + 1, // +1 for date separator
            ),
          );
        }),
  ...
  Map<String, List<BrokerModel>> _groupBrokersByWeek(
      List<BrokerModel> brokers) {
    final Map<String, List<BrokerModel>> grouped = {};

    // Sort brokers by creation date (newest first)
    final sortedBrokers = List<BrokerModel>.from(brokers);
    ...
    for (final broker in sortedBrokers) {
      final date = broker.createdAt ?? DateTime.now();
      final weekKey = _getWeekKey(date);

      if (!grouped.containsKey(weekKey)) {
        grouped[weekKey] = [];
      }
      grouped[weekKey]!.add(broker);
    }
  ...
  Widget _buildErrorWidget(BrokersListViewModel vm, ColorScheme colors,
      TextTheme texts, AppLocalizations loc,
      [String? errorMessage]) {
    return Container(
  ...
  Widget _buildStatisticsCard(List<BrokerModel> brokers, ColorScheme colors,
      TextTheme texts, AppLocalizations loc) {
    final totalBrokers = brokers.length;
    final brokersWithNotes = brokers.where((b) => b.notes.isNotEmpty).length;
      ...
      child: Row(
        children: [
          ...
          Container(
            ...
            child: Column(
              children: [
                const Icon(Icons.note_outlined, color: Colors.white, size: 20),
  ...
  Future<void> _showDeleteConfirmation(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text(
                loc.translate('confirmDeleteBroker'),
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    Expanded(
                      child: Text(
                        broker.name.isNotEmpty ? broker.name : 'Unknown Broker',
                        style: Theme.of(context).textTheme.bodyMedium?.copyWith(
                              fontWeight: FontWeight.w600,
    ...
    if (confirmed == true) {
      if (broker.id != null) {
        await vm.deleteBroker(broker.id!, context);
      }
    }
  }

  // WhatsApp functionality
  Future<void> _openWhatsApp(String phoneNumber, String brokerName) async {
    try {
      ...
      final message = Uri.encodeComponent(
          'Hello $brokerName, I hope you are doing well. I wanted to discuss some business opportunities.');
...
class _EnhancedBrokerTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Dismissible(
      key: Key('broker_${broker.id ?? index}'),
                  ...
                  Row(
                    children: [
                      ...
                      Container(
                        ...
                        child: ClipOval(
                          child: SvgPicture.asset(
                            'assets/icons/Brokers-Avatar.svg',
                            fit: BoxFit.cover,
                            width: 50,
                            height: 50,
                          ),
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                Text(
                                  broker.createdAt != null
                                      ? _getFormattedDate(broker.createdAt!)
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Container(
                          height: 40,
                          decoration: BoxDecoration(
                            color: broker.phoneNumber.isNotEmpty
                          ...
                          child: Material(
                            ...
                            child: InkWell(
                              borderRadius: BorderRadius.circular(12),
                              onTap:
                                  broker.phoneNumber.isNotEmpty ? onCall : null,
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.center,
                                children: [
                                  Icon(
                      ...
                      Expanded(
                        child: Container(
                          ...
                          child: Material(
                            ...
                            child: InkWell(
                              borderRadius: BorderRadius.circular(12),
                              onTap: broker.phoneNumber.isNotEmpty
                                  ? onWhatsApp
                                  : null,
                              child: Row(
                                mainAxisAlignment: MainAxisAlignment.center,
