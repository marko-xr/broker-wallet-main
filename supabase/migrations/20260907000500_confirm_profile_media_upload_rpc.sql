-- ============================================================================
-- Migration: PREPARE atomic profile media confirm RPC for Worker
--
-- Status: PREPARED ONLY — DO NOT APPLY automatically in this pass.
--
-- Why:
-- - Existing hosted schema already provides profile_media_id + RLS baselines.
-- - We need a transaction-safe, idempotent confirm path so Worker retries
--   cannot leave data in a partially-linked state.
-- - No helper view is created here.
--
-- Function contract (service role only):
--   confirm_profile_media_upload(
--     p_user_id uuid,
--     p_media_id uuid,
...
--   )
-- Returns:
--   previous_media_id uuid, profile_media_id uuid
--
-- Apply manually later if approved.
-- ============================================================================
...
begin
  ...
  if p_observed_size is null or p_observed_size <= 0 or p_observed_size > 10485760 then
    raise exception 'Observed object size out of allowed range';
  end if;

  select owner_id, status, content_type
  ...
  if p_observed_content_type is not null
     ...
     and lower(trim(p_observed_content_type)) <> lower(trim(v_expected_content_type)) then
    raise exception 'Observed content type does not match expected media content type';
  end if;
...
revoke all on function public.confirm_profile_media_upload(uuid, uuid, bigint, text) from public;
grant execute on function public.confirm_profile_media_upload(uuid, uuid, bigint, text) to service_role;
